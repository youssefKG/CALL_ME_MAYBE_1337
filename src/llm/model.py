"""Facade around the local language
model SDK used by the pipeline."""
...
class Model(Small_LLM_Model):
    """Wrap the small LLM SDK with convenience
    methods for token and logits handling."""
    ...
    def decode_ids(self, ids: list[int]) -> str:
        ...
        Returns:
            str: Decoded text.
        """
        return cast(str, super().decode(ids))
    ...
    def get_masked_logits(
        ...
        Returns:
            list[float]: Logits with illegal tokens
            suppressed to negative infinity.
        """
